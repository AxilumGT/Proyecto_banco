...
class UsuarioDAO:
    ...
    def iniciar_sesion(cls, username, password):
        txt = ' WHERE username=%s AND password=%s'
        with CursorPool() as cursor:
            cursor.execute(cls._SELECT + txt, (username, password))
    ...
    def update(cls, password, mail, money, id_account):
        with CursorPool() as cursor:
            cursor.execute(cls._UPDATE, (password, mail, money, id_account))
...
if __name__ == '__main__':
    user = UsuarioDAO.select_id_account('7640883a')
    UsuarioDAO.update(user.password, user.mail, 10000.555, '7640883a')
    print(user)

    # UsuarioDAO.update(user.password, user.mail, 1000, user.id_account)
